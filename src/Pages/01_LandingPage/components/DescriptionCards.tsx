...
const cardInfo: cardInfoType = [
  ...
  skill and experience to tackle any of you A/C projects.`, pic: AC, path: '/ac'},
  {title:"REFRIGERATION", desc:`From walk-in freezers to milk tanks, our refrigeration
  service team are industry leaders.`, pic: refrigeration, path:"/refrigeration"},
  {title:"HEATING", desc:'With our experience with furnaces and heating products, we can make sure you stay warm during those cold winter days.', pic: heating, path: "heating"},
  {title:"SERVICES", desc:'Emergency repairs to preventative maintenance, Cam Air is ready to have your equipment running efficiently, so you can run your business be worry free.', pic: tech, path: "services"},
];
...
const cardList = cardInfo.map((card:cardType, i:number)=>{
  ...
  return (
<Link to={card.path} key={i} style={{ display: "flex", justifyContent: "center", textDecoration: 'none' }}>
            <Paper
              elevation={5}
              sx={descriptionCardBackgroundPaperSX}
            >
              <Grid
                container
                direction="row"
              ...
              >
                <Grid item sm={4} xs={12} >
                  <Box
                    sx={{backgroundImage: `url(${card.pic})`, ...descriptionPictureSX}}
                  />
                </Grid>
